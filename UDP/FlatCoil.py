...
primitiveInfo = UDPPrimitiveTypeInfo(
    name = "Flat Coil",
    purpose = "Create a Flat Coil in XY plane",
    company = "Julius Herb",
    date = "4-6-2022",
    version = "1.0")
...
#-----------------------------------------------
    ...
    def CreatePrimitive2(self, funcLib, paramValues):
        ...
        funcLib.AddMessage(MessageSeverity.InfoMessage, "Create FlatCoil with parameters: " + ", ".join(paramData))
        ...
        theUDPSweepOptions = UDPSweepOptions(SweepDraftType.ExtendedDraft, 0.0, 0.0) # RoundDraft
...
#------------------------------------------------
    def _CreatePath(self, funcLib, paramValues):
        ...
        for i in range(1, num_points):
            points_z[i] = start_z #pitch/4. * i - pitch/8.

        points_x[num_points - 1] = points_x[num_points - 2]
        points_y[num_points - 1] = start_y
        points_z[num_points - 1] = start_z #pitch * num_turns
